...
const TimeCheckboxGenerator = ({ selectedTimeZone, apiDate }) => {
    ...
    while (
      ...
      const getValueCheck = () => {
        return apiData.filter((el, i) => {
          return (
            formattedTime.toString() === el.time &&
            formattedApiDate.toString() === el.date
          );
        });
      };
      ...
      checkboxes.push(
        <div key={formattedTime}>
          <input
            type="checkbox"
            name="checkDatas"
            checked={getValueCheck()[0]?.checkData}
            onChange={(e) => handleSubmit(e, formattedTime, formattedApiDate)}
            id={formattedTime}
          />
  ...
  return (
    <Stack direction="row" spacing={2}>
      {formattedApiDate < date ? (
        <p>PAST</p>
      ) : (
        generateTimeIntervals(selectedTimeZone)
      )}
    </Stack>
  );
};
